...
- Per-target points ensure partial success is visible: Cone Control pays 100
  score per cone, up to 4500.
- Cone Control awards Bronze at 30, Silver at 40, and Gold at all 45 cones.
...
Each tenth cone escalates toward the chase camera; cone 45 receives the
...
lane changes, one shoulder-airbrake transfer, and re-centering between inputs.

The lap is a roughly 2.35 km asphalt loop, about 40% longer than the earlier
prototype. Curve strength rises from broad `±1` sweepers to `±2`, then a short
`±2` chicane and one late `-3` curve. Straights separate every major demand.
There is no dirt, elevation test, nitro, zipper, ramp, or rock.

Forty-five cones form five recognizable traffic-control phrases instead of
identical gates: an eight-cone straight smash line, an eight-cone merge taper,
two four-cone staggered work-zone lines, a nine-cone left-to-right transfer, and
a twelve-cone finishing slalom.
Small offset changes in the straight, taper, and slalom reward ordinary steering
rather than demanding an airbrake on every read. Only the transfer explicitly
invites L1/R1.
...
```json
{
  ...
  "scoring": {
    ...
    "thresholds": [
      { "rank": "gold", "minimum": 45, "stars": 3 },
      { "rank": "silver", "minimum": 40, "stars": 2 },
      { "rank": "bronze", "minimum": 30, "stars": 1 }
    ]
  },
  "objects": [
    {
