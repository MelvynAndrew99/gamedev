...
- [x] Syndicate Run rival difficulty, can it be beat even with a perfect run?
  - Yes, and the later hands-on balance pass made that proof player-realistic: the fastest rival now projects to about 178s versus a 151s perfect ceiling, requiring roughly 85% of the ceiling rather than 95%.
...
- [x] Night Drive is better than my Chrome & Credits, and you rework the music for Chrome & Credits, but leave the original for me to have the original song in the music player?  In fact update the md files to say that original songs should be available in the music player :)
  - Superseded by the later request to keep only the stronger production mixes; the duplicate drafts and old preservation rule were removed.
...
- [x] Use a music expert agent to evaluate story mode tracks.  Needs balancing and updates to match better production songs we have from Night Drive.  Each track has a style that should be preserved, but looking at proper EQ so my bass doesn't blast my speakers and we can hear the mids and highs or melody better.  Catchy things we can dance to or get stuck in your head.
  - The specialist pass added per-song bass band limiting and bounded voice gains, brought Neon Gulch and Syndicate hooks forward without erasing their future-funk/cyber-metal identities, and authored Proving Ground's hook-driven D-Dorian “Start Signal.” The later library cleanup keeps only these strongest production mixes.
...
- [x] Music Player graphics need fixing to have scrollable music ![player](./images/music_player.png)
  - The soundtrack library uses a six-row scroll window with keyboard/controller/wheel navigation, a proportional rail and thumb, range count, and contained pointer rows. Its first and final windows were verified at 800×600.
...
- [x] The biggest overhaul I need now is the character sprite, which I am including the inspiration for what I think the game needs ![vehicle](./images/vehicle.png), this one needs the strictest validation gate/agent to make sure that the perspectives match the current sprite and that we can modify colors for rivals.  The key to making th is work is using the image generation skills to make it look better than a place holder.  So if we can make this visualization or graphics update work with the feel of the game, then we can make the swap.  This is complete when the perspectives match from a tailing perspective, we need both hard and soft turns so we can visualize the air breaks and turning along with the pitch up and down left and right for the ramps and the flight school.  Sprite should be as detailed as we can for the game as the player will be looking at it for all of the gameplay.  Again the idea is that we can use different colors for rivals.
  - Replaced the placeholder with the original detailed Pulsewing: 15 rear-chase poses cover soft/hard mirrored steering and three pitch rows. Rival liveries recolor only the hull while glass, engines, metal, stripe, and outlines stay authored. The deterministic source pipeline, nine 800×600 captures, and strict independent gate all pass.
- [x] Tracks would benefit from a landmark and better scenery as we are racing through them.  Small objects that we drive by give a sense of speed so I want to keep them, but there isn't much to look at from a surroundings perspective.  Can you look at how other games handle filling in the screen real estate with beautify visuals that make the world feel real or immersive?  School track can be minimalistic as we want it to feel like a training course designed for learning.
  - Each world now combines layered parallax scenery, frequent peripheral speed cues, and rarer signature silhouettes: test-circuit infrastructure, desert energy landmarks, and freight-belt structures. Race School deliberately uses smaller, sparser scenery. All four were reviewed at speed at 800×600.
- [x] Delete original music from the game and music player, your edits are better and are thematically the same just better balancing.
  - Removed the three duplicate ORIGINAL entries and their exported draft arrangements. The library now exposes one production mix per composition, and the music design rules match that decision.
- [x] New vehicle design needs shadow under it, just makes the car feel more real.
  - A grounded contact shadow now follows lateral movement and shrinks/fades beneath airtime so height remains readable.
- [x] All gold in drive school unlocks car editor.  Really just lets you change color.
  - Gold in the five normal driving lessons permanently unlocks a six-color Garage editor; secret Flight School is intentionally excluded. The chosen hull livery persists into the title and races while fixed glass, engines, lights, and trim retain their authored materials.
- [x] Trophy room needs a date earned info so it can feel like a steam achievement.
  - School medals now store and show the date their current rank was first earned. Same-rank replays preserve it, upgrades get their own date, and old saves truthfully say LEGACY SAVE rather than fabricating history.
- [x] Syndicate Run Rival race needs balancing.  I can't keep up with the rivals yet alone take them out.  I love the track design, especially since you are leveraging things learned in the school like air break into a boost pad because a rock is in the way and using the down pitch after a jump so you don't over jump the road.  These are fantastic design choices and makes its really satisfying to move through the course as it requires mastery and flow.  The only issue is I can't catch the rivals and there is not enough boosts to really boost kill the rivals.  I am glad it is hard, but it shouldn't be impossible.
  - Reduced the three-lap pack's sustained pace and opening gaps without flattening the finale: winning still requires about 106% of normal top speed. Four extra bankable boosts now lead directly into the four clean attack straights, giving every lap deliberate takedown opportunities.
- [x] Left and right turns on the vehicle are reversed so you are turning the sprite the wrong direction.  It should just be flipped, and then bravo this is a really big deal that you got this right.
  - Mirrored the rear-chase pose lookup for the player, rivals, and Air School demonstrations without changing steering physics.
- [x] Vehicle on title screen needs scaling down to match building sizes more appropriately.  I don't mind exaggerated dimensions, but its a tad too big.
  - Reduced the hero from 1.12× to 0.98× and retuned its contact shadow; the final 800×600 composition keeps it grounded while letting the city retain scale.
- [x] Up and down on the jumps for the sprite is also backwards.  Just need to flip it.
  - Reversed only the visual pitch-row lookup, so push-for-short and pull-for-long physics stay intact while the nose silhouette now follows the chosen arc.
- [x] Moving road in the title screen is also showing up in the vehicle rendering, vehicle doesn't need that scolling effect.
  - Animated road crossbars now split around a protected hero-car zone; the road still moves on both sides without scrolling through transparent engine or underbody gaps.
- [x] Add alpha channel to selection boxes on the title menu so we can see the road animation better.  Its really slick but most of it is covered by grey boxes.
  - Main destination backplates now use 62% opacity at rest and 78% when selected, leaving the animated road visible while preserving strong text, icon, border, and focus contrast. Submenu panels remain opaque for readability; verified at 800×600.
