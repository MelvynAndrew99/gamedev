...
export class Player {
  constructor(tuning) {
    ...
    // The sequel-preview flight is activated only by GameScene's exact
    // training-flight gate. Ordinary ramps never touch this flag.
    this.sustainedFlight = false;
  }

  get airborne() {
    return this.sustainedFlight || this.air > 0;
  }

  // 0 at takeoff/landing, 1 at apex — for the sprite's visual arc.
  get airArc() {
    if (this.sustainedFlight) return 0.35;
  ...
  beginSustainedFlight({ boosted = false } = {}) {
    this.sustainedFlight = true;
    this.air = 0;
    this.airTotal = 0;
    this.launchSpeed = this.speed;
    this.boostedLaunch = boosted;
  ...
  update(dt, input, model) {
    ...
    const grip = this.sustainedFlight ? 0.72 : this.airborne ? 0.25 : 1;
    ...
    // Sustained Flight School movement is authored around free aerial control:
    // the road bends below as a reference, but cannot centrifugally shove an
    // aircraft that is no longer touching it. Finite jumps retain the racing
    ...
    if (this.sustainedFlight && input.brake > 0)
      this.speed += t.braking * 0.45 * dt * input.brake;
    else if (input.throttle > 0 && this.speed < t.maxSpeed)
...
// the synchronized car-v2 atlases generated by tools/gen-car.js. This file is
// pure handling logic: no Phaser, no DOM, no pixels. Testable in plain Node.)
